...
export const useGsapAnimations = () => {
  useEffect(() => {
    const hasTargets = (selector: string) =>
      gsap.utils.toArray(selector).length > 0;
    ...
    if (hasTargets('.hero-title, .hero-subtitle, .hero-cta')) {
      const heroTimeline = gsap.timeline({ delay: 0.3 });

      if (hasTargets('.hero-title')) {
        heroTimeline.fromTo(
          '.hero-title',
